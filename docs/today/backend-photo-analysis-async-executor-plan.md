...
> 단, prefix는 **로그 패턴에 `%thread`가 있어야** 효과가 있다. 실측 결과 이 프로젝트의 `application.properties` `logging.pattern.console`이 스레드명을 빼고 있어, prefix를 줘도 평소 콘솔 로그엔 안 찍혔다. → 패턴에 `[%thread]`를 추가해야 관찰가능성 이점이 실제로 살아난다. (스레드 덤프에는 패턴과 무관하게 prefix가 그대로 보인다.)
...
- [x] **조각 1 — `config/AsyncConfig.java` 신규: `photoAnalysisExecutor` 빈**
...
- [x] `./gradlew bootRun` 후 사진 업로드 → 로그 스레드 이름이 `photo-analysis-*`로 찍히는지 확인
- [x] 사진 여러 장 동시 업로드 → 풀 size만큼만 동시에 돌고 나머지는 큐에서 순차 처리되는지 확인
- [x] 큐 한계를 넘는 양을 던졌을 때 거부 정책대로 동작하는지(`CallerRunsPolicy`면 호출 스레드 실행, `AbortPolicy`면 예외) 확인
- [x] 분석 실패 사진이 여전히 `FAILED`로 기록되고 재분석 UI가 뜨는지 (기존 동작 회귀 없음)
...
 AbortPolicy 전환은 기존 방식이 업로드 응답이 분석 떄문에 느려지면 실행