...
- [x] 새로고침 후에도 진행 중 상태가 이어지는지 확인한다.
...
- [x] `#2` 검증: 백엔드를 내리거나 네트워크를 차단했을 때 3회 실패 후 멈추고 에러 안내가 뜨는지 확인한다.
- [x] `#3` 검증: 응답을 계속 `PENDING`으로 유지했을 때 상한 도달 후 타임아웃 안내가 뜨고 루프가 멈추는지 확인한다.
- [x] `#1` 검증: 분석 중 새로고침했을 때 사진과 진행 상태가 복원되고 폴링이 이어지는지 확인한다.
- [x] 공통 검증: 재분석 후에는 카운터가 초기화된 상태로 다시 추적되는지 확인한다.
- [x] 공통 검증: `stopPolling()` 이후 타이머가 남지 않는지 확인한다.

---

## 5. 연관 변경

- [x] `ReviewPostService.createReview` 멱등화(get-or-create)가 `#1` 복원의 데이터 출처인지 확인한다.
- [x] 같은 plan으로 다시 들어와도 서버에 남아 있는 draft를 기준으로 사진 상태를 복원할 수 있는지 확인한다.

---

## 6. 다듬기

- [x] 재분석 성공 시 `stopPolling()` 후 `startPolling()` 재시작 흐름을 더 매끈하게 다듬는다.
- [x] 재분석 직후 `idle`을 거치는 UI 깜빡임이 있는지 확인한다.
- [x] 재분석 중에는 이전 실패 알림이 잠깐 보이지 않도록 표시 우선순위를 정리한다.
- [x] `handleReanalyze()`의 상태 전이를 주석으로 더 명확하게 남긴다.
